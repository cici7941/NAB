...
#functions
getAlertTypes,
getClosestPrecedingWindow,
getFirstTruePositive,
getWindows,
getScore,
normalizeScore,
scoreDataSet
...
```
"""
function __repr__(window::Window)
...
```
"""
function getWindows(scorer::Scorer, limits::AbstractArray{Tuple{DateTime,DateTime},1})
...
function scoreDataSet(
    ...
    if isempty(costMatrix)
        profiles = JSON.parsefile(joinpath(dirname(@__FILE__), "profiles.json"))
        try
        ...
        if any(weight -> !(weight in keys(costMatrix)), ["fnWeight", "fpWeight", "tpWeight"])
