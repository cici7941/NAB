...
type Labeler
    ...
    function Labeler(windowSize::Float64, probationaryPercent::Float64)
        self = new(
                DataFrame(), windowSize, probationaryPercent,
                DataFrame(), Int[], Tuple{DateTime,DateTime}[],
...
function setLabels(labeler::Labeler, trueAnomalies::AbstractArray{DateTime, 1})
    labels = labeler.data[[:timestamp]]
    labels[:label] = collect(Int, map(t -> t in trueAnomalies, labeler.data[:timestamp]))
    labeler.labelIndices = find(Array(labels[:label]))
...
function applyWindows(labeler::Labeler)
    ...
    for a in labeler.labelIndices
        ...
        windowLimit = (labeler.data[:timestamp][front], labeler.data[:timestamp][back])
...
function checkWindows(labeler::Labeler)
    ...
    if numWindows > 0
        ...
        probationTimestamp = labeler.data[:timestamp][probationIndex]

        if labeler.windows[1][1] < probationTimestamp
            deleteat!(labeler.windows, 1)
            Logging.info("""The first window overlaps with the probationary period,
