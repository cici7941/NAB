...
export
convertAnomalousWindowsToTimestamps,
...
```
"""
function convertAnomalousWindowsToTimestamps{T<:TimeType}(anomalousWindows::AbstractVector{Tuple{T,T}})
    trueAnomalies = Array(DateTime, 0)
    for window in anomalousWindows
        startTime, endTime = window
        startTime, endTime = DateTime(startTime), DateTime(endTime)
